...
pattern = re.compile(
    r"Performance:\s+([0-9.]+)\s+ns/day,\s+([0-9.]+)\s+hours/ns,\s+([0-9.]+)\s+timesteps/s,\s+([0-9.]+)\s+Matom-step/s"
)
        ...
        with open(log_path, "r") as f:
            ...
            for line in f:
                if line.startswith("Performance:"):
                    ...
                    if match:
                        ns_per_day, hrs_per_ns, steps_per_s, Matom_steps_per_s = map(float, match.groups())
                        results[folder] = {
                            "ns/day": ns_per_day,
                            "hours/ns": hrs_per_ns,
                            "timesteps/s": steps_per_s,
                            "Matom-step/s": Matom_steps_per_s,
                        }
                    break
...
header = "gpu ns/day hours/ns timesteps/s Matom-step/s s/timestep"
